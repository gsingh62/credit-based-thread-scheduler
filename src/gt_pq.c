...
/*load balancing function-under*/
int check_For_Other_kthread_uthreads_under()
{
	printf("check_for_other_kthread_uthreads_under\n");
	kthread_context_t * k_ctx;
	runqueue_t * runq;
	prio_struct_t *prioq;
	uthread_head_t *u_head;
	uthread_struct_t *u_obj;
	unsigned int uprio, ugroup, num_cpus, inx, current_kthread_idx = kthread_apic_id();
	kthread_runqueue_t * kthread_runq;
	kthread_runqueue_t * current_kthread;
	k_ctx = kthread_cpu_map[current_kthread_idx];
	current_kthread = &(k_ctx->krunqueue);

	num_cpus = (int)sysconf(_SC_NPROCESSORS_CONF);

	for(inx=1; inx<num_cpus; inx++)
	{
		if(inx == current_kthread_idx)
			continue;

		k_ctx = kthread_cpu_map[inx];
		...
		if(runq->uthread_mask)
		{
			//extract next highest priority queue			
			uprio = LOWEST_BIT_SET(runq->uthread_mask);
			prioq = &(runq->prio_array[uprio]);
			assert(prioq->group_mask);
			ugroup = LOWEST_BIT_SET(prioq->group_mask);
			...
			//schedule it on this kthread'sunder run queue;
			add_to_runqueue(current_kthread->active_runq, &(current_kthread->kthread_runqlock), u_obj);
			current_kthread->uthread_count++;
...
/*load balancing function-over*/
int check_For_Other_kthread_uthreads_over()
{
	printf("check_for_other_kthreads_uthreads_over\n");
	kthread_context_t * k_ctx;
	runqueue_t * runq;
	prio_struct_t *prioq;
	uthread_head_t *u_head;
	...
	for(inx=1; inx<num_cpus; inx++)
	{
		...
		if(runq->uthread_mask)
		{
			...
			rem_from_runqueue(runq, &(kthread_runq->kthread_runqlock), u_obj);
			//schedule it on this kthread'sunder run queue;
			add_to_runqueue(current_kthread->expires_runq, &(current_kthread->kthread_runqlock), u_obj);
			current_kthread->uthread_count++;
...
{	printf("sched_find_best_uthread_credit\n");
	...
	printf("cpu(%d) : sched best uthread(id:%d, group:%d)\n", u_obj->cpu_id, u_obj->uthread_tid, u_obj->uthread_gid);
#endif
	printf("Exiting find best  uthread credit\n");
	return(u_obj);
#if 0
	runqueue_t *runq;
	prio_struct_t *prioq;
...
search_for_uthread_under:
	printf("search_for_uthread_under\n");
	runq = kthread_runq->active_runq;
	kthread_runq->kthread_runqlock.holder = 0x04;
	if(!(runq->uthread_mask))
	{ /* No jobs in under. switch runqueue */
		assert(!runq->uthread_tot);

		//if(check_For_Other_kthread_uthreads_under())				
			//goto search_for_uthread_under;
		
		

search_for_uthread_over:
		printf("search_for_uthread_over");
		runq = kthread_runq->expires_runq;
		if(!(runq->uthread_mask))
		{
			//assert(!runq->uthread_tot);
			//if(check_For_Other_kthread_uthreads_over())
				//goto search_for_uthread_over;
			return NULL;
		}		
	}
